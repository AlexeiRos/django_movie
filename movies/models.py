...
class Category(models.Model):
    ...
    def get_absolute_url(self):
        return reverse('sidebar', kwargs={'slug': self.slug})
...
class Serial(models.Model):
    ...
    country = models.CharField("Страна", max_length=30)
    directors = models.ManyToManyField(Actor, verbose_name="режиссер", related_name="serial_director")
    actors = models.ManyToManyField(Actor, verbose_name="актеры", related_name="serial_actor")
    genres = models.ManyToManyField(Genre, verbose_name="жанры")
    world_premiere = models.DateField("Примьера в мире", default=date.today)
    time_series = models.CharField("Длительность серии", max_length=100,)
    category = models.ForeignKey(
        Category, verbose_name="Категория", on_delete=models.SET_NULL, null=True
    )
    slug = models.SlugField(max_length=130, unique=True)
    ...
    def get_absolute_url(self):
        return reverse("serial_detail", kwargs={"slug": self.slug})
...
class ReviewsSerial(models.Model):
    """Отзывы к сериалу"""
    email = models.EmailField()
    name = models.CharField("Имя", max_length=100)
    text = models.TextField("Сообщение", max_length=5000)
    ...
    serial = models.ForeignKey(Movie, verbose_name="сериал", on_delete=models.CASCADE)
